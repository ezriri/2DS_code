...
# also worked out i was recording the wrong time, and now fixed that. pretty much most of particles have same start/ end time
...
@jit(nopython=True)
def start_end_time_particle(a_slice_ss_s, a_slice_ss_e, full_xcoords, slice_crystal):
# provide the slice start and end times, xcoords of the particle, and the 
    if a_slice_ss_s == a_slice_ss_e:
        return a_slice_ss_s, a_slice_ss_e
    else:
        time_dif = a_slice_ss_e - a_slice_ss_s # how many seconds was this slice?
        height, slice_width = one_crystal.shape # how many pixels across is this change in time
        if slice_width == 0:
            raise ValueError("slice_width must be greater than zero.")
        pixel_time_length = time_dif/slice_width # 1 pixel = how many .seconds
        particle_start_t = a_slice_ss_s + (full_xcoords[0]*pixel_time_length)
        particle_end_t = a_slice_ss_s + (full_xcoords[-1]*pixel_time_length)
        return particle_start_t, particle_end_t
...
# 3. loop: processing each slice  -    -    -    -    -    -    -    -    -    -    -    -    -    -    -    -    -    -    -    -    -    -    -    -    -    -    -    -    -    -    -    -    -    -    -    -    -
        for i in range(len(selected_pix_sum)-2):        
            ...
            one_crystal = h5_file['ImageData'][:,int(selected_pix_sum[i]):int(selected_pix_sum[i+1])] # extract 1 slice (how manc defined a crystal)
...
# only particles making it past this point will be recorded
                            ...
                            if euclidean_dim * pixel_resolution >= length_threshold:
                                ...
                                slice_ss_e = float(selected_ss_time[i+1].values)
                                ...
                                one_particle_data = {
                                        ...
                                        "ss_start": p_start_t, #time_xr['sec_since'][s_idx].values, # seconds since midnight
                                        "ss_end": p_end_t, #time_xr['sec_since'][e_idx].values, # seconds since midnight
                                        "x_coords": x_coords,
                                        "y_coords": y_coords,
