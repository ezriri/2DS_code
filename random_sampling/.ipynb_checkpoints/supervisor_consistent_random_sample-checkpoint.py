...
# this code works, but need to check is whole numbers between total sample + number of secondary labeling people
...
def save_csv(image_list, save_full_path):
    ...
    csv_df.to_csv(save_full_path, index=False)
...
## save this as csv for ease
whole_sample_name = f'{base_path_to_save}full_{str(n_sample)}_sample.csv'
save_csv(r_sample_list,whole_sample_name)
...
for supervisor_number in final_lists:
    ...
    ## csv saving
    save_csv(set_of_images, csv_save_path)
